...
from sklearn.metrics import balanced_accuracy_score, roc_auc_score as ras
import numpy as np
...
def trainNetwork(net, trainloader, validloader, testloader, model_path=None, iterations=500, lr=5*1e-4, wd=None, repeat=None, sub=None, epochs=None):
    ...
    for ite in range(iterations):
        ...
        for xb, yb in trainloader:
            tr_len += yb.shape[0]
            out = net(xb)
            # print("output of net ",out.shape)
            # print("label ",yb.shape)

            loss = CE(out, yb).double()
...
def testNetwork(net, testloader):
    ...
    for xb, yb in testloader:
        with torch.no_grad():
            pred = net(xb)
            predicted = torch.argmax(softmax(pred))
    ...
    return balanced_accuracy_score(np.array(Y_test),np.array(y_pred))
