...
def compute_ref_chordal(m_ra, m_ea, alpha=0.5):

    r_mix = (1-alpha) * m_ra + alpha * m_ea
...
def compute_chordal(data, m_ra=None, m_ea=None, alpha=0.5, dtype='covmat', r_hre=None):
    # dtype (data type) could be covmat or raw
    data = copy.deepcopy(data)

    if r_hre is None:
        r_hre = compute_ref_chordal(m_ra, m_ea, alpha=alpha)
...
def chordal_alignment(data, list_mean_r, list_mean_e, alpha=0.5, dtype='covmat', domains=None, estimator='lwf'):
    data_aux = []
    ...
    if domains is None:
        ...
        for k in range(int(n_means)):
            ...
            m_r = list_mean_r[k]
            m_e = list_mean_e[k]
            batch_HA = compute_chordal(batch, m_ra=m_r, m_ea=m_e, alpha=alpha, dtype=dtype)
