...
jobstring1=$(sbatch new_dataset_1.sh)
...
jobstring2=$(sbatch --dependency=afterany:${jobid1} new_dataset_2.sh)
jobid2=${jobstring2##* }

# Now submit j3.slurm as a dependant job to j2:

jobstring3=$(sbatch --dependency=afterany:${jobid2} new_dataset_3.sh)
jobid3=${jobstring3##* }

# Now submit j4.slurm as a dependant job to j3:

jobstring4=$(sbatch --dependency=afterany:${jobid3} new_dataset_4.sh)
jobid4=${jobstring4##* }

# Now submit j5.slurm as a dependant job to j4:

jobstring5=$(sbatch --dependency=afterany:${jobid4} new_dataset_5.sh)
jobid5=${jobstring5##* }

# Now submit j6.slurm as a dependant job to j5:

jobstring6=$(sbatch --dependency=afterany:${jobid5} new_dataset_6.sh)
jobid6=${jobstring6##* }

# Now parralel work with j7:

jobstring7=$(sbatch new_dataset_7.sh)
jobid7=${jobstring7##* }

# Now submit j8.slurm as a dependant job to j7:

jobstring8=$(sbatch --dependency=afterany:${jobid7} new_dataset_8.sh)
jobid8=${jobstring8##* }

# Now submit j9.slurm as a dependant job to j8:

jobstring9=$(sbatch --dependency=afterany:${jobid8} new_dataset_9.sh)
jobid9=${jobstring9##* }

# Now submit j10.slurm as a dependant job to j9:

jobstring10=$(sbatch --dependency=afterany:${jobid9} new_dataset_10.sh)
jobid10=${jobstring10##* }

# Now submit j11.slurm as a dependant job to j10:

jobstring11=$(sbatch --dependency=afterany:${jobid10} new_dataset_11.sh)
jobid11=${jobstring11##* }

# Now submit j12.slurm as a dependant job to j11:

jobstring12=$(sbatch --dependency=afterany:${jobid11} new_dataset_12.sh)
jobid12=${jobstring12##* }