"""
Copy https://github.com/pog87/PtitPrince

Changes:
  - Adds the alpha parameter to boxes (see https://github.com/pog87/PtitPrince/issues/40)
  - Fixes point height (see https://github.com/pog87/PtitPrince/issues/43)
  - Adds box colour even without hue
    ...
    def draw_stripplot(self, ax, kws):
        ...
        for i, group_data in enumerate(self.plot_data):
            if self.plot_hues is None or not self.dodge:
                ...
                if self.orient == "v":
                    ax.scatter(cat_pos, strip_data, **kws, alpha=self.alpha)
                else:
                    ax.scatter(strip_data, cat_pos, **kws, alpha=self.alpha)
...
def RainCloud(x=None, y=None, hue=None, data=None,
    ...
    kwcloud = dict()
    kwbox = dict(saturation=1, whiskerprops={'linewidth': 2, "zorder": 10}, medianprops={"color": "k", "linewidth": 2},)
