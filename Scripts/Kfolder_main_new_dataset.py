...
def get_data(subjects,tospd=False,recenter=True):
    ...
    if tospd:
        X_riem = np.zeros((data_list.shape[0],length*data_list.shape[1],32,32))
        y_riem = np.zeros((data_list.shape[0],length*data_list.shape[1]))
        for ind_i,i in enumerate(participants):
            ...
            temp_X = Euc2SPD(temp_X,y_riem[ind_i])
            if recenter:
                X_riem[ind_i] = compute_riemannian_alignment(temp_X, mean=None, dtype='covmat')
    else:
        X_riem = None
        y_riem = None
    X = np.zeros((data_list.shape[0],length*data_list.shape[1],data_list.shape[2],int(window_size*sfreq)))
    y = np.zeros((data_list.shape[0],length*data_list.shape[1]))
    for ind_i,i in enumerate(participants):
        events, event_id = mne.events_from_annotations(raw_eeglab[ind_i])
        # print(event_id)
    ...
    return X,y,X_riem,y_riem,labels_code_list,codes



def main(subjects,subtest,recenter=True):
    ...
    X_parent_cnn,Y_parent_cnn,X_riem,Y_riem,labels_codes,codes = get_data(subjects,tospd=True)
    

    # CNN Kfolder
    print("Perform CNN\n")
    cnn_kf = Kfolder("CNN",kfold)
    X_parent_cnn = np.expand_dims(X_parent_cnn,2)
    Y_parent_cnn = np.array([np.vstack((c_k,np.abs(1-c_k))).T for c_k in Y_parent_cnn])
    cnn_kf.perform_Kfold(X_parent_cnn,Y_parent_cnn,labels_codes,codes,subjects,subtest,prefix,n_class=5,window_size=0.25)
    ...
    # # SPD Kfolder
    print("Perform SPD\n")
    spd_kf = Kfolder("SPD",kfold)
    spd_kf.perform_Kfold(X_riem,Y_riem,labels_codes,codes,subjects,subtest,prefix,n_class=5,window_size=0.25)
    
    # # SPDBN Kfolder
    print("Perform SPDBN\n")
    spdbn_kf = Kfolder("SPDBN",kfold)
    spdbn_kf.perform_Kfold(X_riem,Y_riem,labels_codes,codes,subjects,subtest,prefix,n_class=5,window_size=0.25)
...
if __name__ == '__main__':
    ...
    main(args.subjects,args.subtest,args.recenter)
