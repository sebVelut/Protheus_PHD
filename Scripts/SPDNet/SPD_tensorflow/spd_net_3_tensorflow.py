...
class SPDNet2_AJD(Model):
    def __init__(self, batch_size=32, n_epochs=300, valid_split=0.2, 
        ...
        self.d_1 = DiagLayer()
        self.s_1 = SpectralConvLayer(n_centroids=4, in_ch=1, out_ch=2, n_diag=64)

        self.c_2 = CentroidLayer(in_ch=2)
        self.d_2 = DiagLayer()
        self.s_2 = SpectralConvLayer(n_centroids=4, in_ch=2, out_ch=4, n_diag=64)

        self.c_3 = CentroidLayer(in_ch=4)
        self.d_3 = DiagLayer()
        self.s_3 = SpectralConvLayer(n_centroids=4, in_ch=4, out_ch=8, n_diag=64)
    ...
    def fit_and_predict(self, X, y, X_test, y_test, output_path):
        ...
        y_train = y[0:split_p]
        c_train = np.zeros_like(y_train)

        c_train_0 = np.where(y_train == 0)[0]
        c_train[c_train_0[0:(len(c_train_0) // 2)]] = 0
        c_train[c_train_0[(len(c_train_0) // 2):]] = 1

        c_train_1 = np.where(y_train == 1)[0]
        c_train[c_train_1[0:(len(c_train_1) // 2)]] = 2
        c_train[c_train_1[(len(c_train_1) // 2):]] = 3
