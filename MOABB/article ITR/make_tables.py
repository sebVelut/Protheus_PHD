...
for paradigm in ["SSVEP", "P300"]:
    df = pd.read_csv(parent_path / f'article ITR/results_{paradigm}_all.csv',sep=";")
    #if paradigm in ["lhrh", "All", "rf"]:
    #   df_deep = df[df['pipeline'].isin(deep_methods)]
    #   create_latex_table(df_deep, f"{paradigm}-Deep", base_path)

    #   df_raw = df[df['pipeline'].isin(raw_methods)]
    #   create_latex_table(df_raw, f"{paradigm}-Raw", base_path)

    #    df_riemm = df[df['pipeline'].isin(riemannian_methods)]
    #   create_latex_table(df_riemm, f"{paradigm}-Riemannian", base_path)
    #else:
    create_latex_table(df, f"{paradigm}", base_path)