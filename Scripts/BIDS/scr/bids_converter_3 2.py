...
def create_phen_config(bids_root):
    subj_cfg = {}

    subj_cfg['participant_id'] = {}
    subj_cfg['participant_id']['Description'] = "Unique participant identifier"
    subj_cfg['participant_id']['Description'] = "Unique participant identifier"
    subj_cfg['DES'] = {}
    subj_cfg['DES'][
    ...
    if not os.path.exists(bids_root):
        os.makedirs(bids_root)
    with open(op.join(bids_root, "phenotype/des.json"), "w") as outfile:
        json.dump(subj_cfg, outfile, indent=3)


def create_participants_config(bids_root):
    ...
    if not os.path.exists(bids_root):
        os.makedirs(bids_root)
    with open(op.join(bids_root, "participants_2.json"), "w") as outfile:
        json.dump(subj_cfg, outfile, indent=3)

def create_session_config(bids_sessionpath, participant):
    ses_cfg = {}

    ses_cfg["session_id"] = {}
    ses_cfg["session_id"]['Description'] = "Session number"
    ses_cfg['Date'] = {}
    ...
    with open(op.join(bids_sessionpath, "sub-"+participant+"_sessions.json"), "w") as outfile:
        json.dump(ses_cfg, outfile,indent=3)
        
        
        
def create_run_config(bids_runpath, participant, session, task, simuprefix, run):
    run_cfg = {}
    ...
    with open(op.join(bids_runpath,"sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.json"), "w") as outfile:
...
df_behavioral = df_behavioral.rename(columns={"Participant":"participant_id", "Age":'age', "Sex":'sex', "Session":'session_id', "Batch":"Run"})
fields_subj = ["participant_id", 'age', 'sex', 'Laterality', 'Education', 'VideogameQ1', 'VideogameQ2', 'VideogameQ3', 'VideogameQ4', 'Flight_experience']
fields_phen = ["participant_id", 'DES']

fields_session = ['session_id', 'Date', 'Hour', 'VASF', 'ChalderFS', 'Environment']
fields_run = ['Run', 'KSS(before)', 'Flickers', 'NASA_Easy', 'NASA_Hard', 'ChalderFS', 'Workload_Order', 'Notes', 'Performance_Easy', 'Performance_Hard']
    ...
    for session in sessions:
        for run in runs:
            for task in ["calib", "matb"]:
                ...
                df_line = df_behavioral.loc[(df_behavioral['participant_id'] == int(participant[1:])) & (
                            df_behavioral["session_id"] == int(session)) & (df_behavioral['Run'] == int(run))]
                ...
                if os.path.isfile(op.join(path + participant + "/" + simuprefix + "/",
                    ...
                    df_participants = df_behavioral.loc[(df_behavioral['participant_id'] == int(participant[1:]))][
                        fields_subj].dropna()
                    df_phen = df_behavioral.loc[(df_behavioral['participant_id'] == int(participant[1:]))][fields_phen].dropna()
                    df_session = df_behavioral.loc[(df_behavioral['participant_id'] == int(participant[1:])) & (df_behavioral["session_id"] == int(session))][fields_session].dropna()
                    df_run = df_line[fields_run]

                    ### Save demographic and behavioral data

                    if not os.path.isfile(op.join(bids_root, "participants_2.json")):
                        create_participants_config(bids_root)
                    if os.path.isfile(op.join(bids_root, "participants_2.tsv")):
                        df_participants_previous = pd.read_csv(op.join(bids_root, "participants_2.tsv"), sep='\t')
                        if df_participants['participant_id'].values[0] not in df_participants_previous['participant_id'].values:
                            df_participants_updated = pd.concat([df_participants_previous, df_participants]).sort_values(by=['participant_id'])
                            df_participants_updated.to_csv(op.join(bids_root, "participants_2.tsv"), sep='\t', index=False)
                    else:
                        df_participants.to_csv(op.join(bids_root, "participants_2.tsv"), sep='\t', index=False)


                    phen_path = op.join(bids_root, "phenotype")
                    if not os.path.isfile(op.join(bids_root, "phenotype/des.json")):
                        create_phen_config(bids_root)
                    if os.path.isfile(op.join(bids_root, "phenotype/des.tsv")):
                        df_phen_previous = pd.read_csv(op.join(bids_root, "phenotype/des.tsv"), sep='\t')
                        if df_phen['participant_id'].values[0] not in df_phen_previous['participant_id'].values:
                            df_phen_updated = pd.concat([df_phen_previous, df_phen]).sort_values(by=['participant_id'])
                            df_phen_updated.to_csv(op.join(bids_root, "phenotype/des.tsv"), sep='\t', index=False)
                    else:
                        df_phen.to_csv(op.join(bids_root, "phenotype/des.tsv"), sep='\t', index=False)

                    ses_path = op.join(bids_root, "sub-" + participant)
                    if not os.path.isfile(op.join(ses_path, "sub-"+participant+'_sessions.json')):
                        create_session_config(ses_path, participant)
                    if os.path.isfile(op.join(ses_path, "sub-"+participant+'_sessions.tsv')):
                        df_session_previous = pd.read_csv(op.join(ses_path, "sub-"+participant+'_sessions.tsv'), sep='\t')
                        if df_session["session_id"].values[0] not in df_session_previous["session_id"].values:
                            df_session_updated = pd.concat([df_session_previous, df_session]).sort_values(by=["session_id"])
                            df_session_updated.to_csv(op.join(ses_path,  "sub-"+participant+'_sessions.tsv'), sep='\t', index=False)
                    else:
                        df_session.to_csv(op.join(ses_path, "sub-"+participant+'_sessions.tsv'), sep='\t', index=False)


                    run_path = op.join(bids_root, "sub-" + participant, "ses-" + session, "beh")
                    if not os.path.isfile(op.join(run_path, "sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.json")):
                        create_run_config(run_path, participant, session, task, simuprefix, run)
                    if os.path.isfile(op.join(run_path, "sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.tsv")):
                        df_run_previous = pd.read_csv(op.join(run_path, "sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.tsv"), sep='\t')
                        if df_run['Run'].values[0] not in df_run_previous['Run'].values:
                            ...
                            df_run_updated.to_csv(op.join(run_path, "sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.tsv"), sep='\t', index=False)
                    else:
                        df_run.to_csv(op.join(run_path, "sub-"+participant+"_ses-"+session+"_task-"+task+simuprefix+"_run-"+run+"_beh.tsv"), sep='\t', index=False)
...
df_participants = pd.read_csv(op.join(bids_root, "participants_2.tsv"), sep='\t')
ps = [p for p in df_participants["participant_id"].values]
df_participants = df_participants.replace(ps, ["sub-s"+str(p) for p in ps])
df_participants[df_participants.columns[0:]].to_csv(op.join(bids_root, "participants.tsv"), sep='\t', index=False)

with open(op.join(bids_root, "participants_2.json"), "r") as infile:
    pj = json.load(infile)

with open(op.join(bids_root, "participants.json"), "w") as outfile:
    json.dump(pj, outfile, indent=3)
