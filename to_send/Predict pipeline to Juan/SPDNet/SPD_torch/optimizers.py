...
class MixOptimizer_:
    ...
    def __init__(self, parameters, optimizer=torch.optim.SGD, lr=1e-2, *args, **kwargs):
        ...
        self.stiefel_block_parameters = [param for param in parameters if
        ...
        self.weight_vector_parameters = [param for param in parameters if
    ...
    def step(self):
        ...
        loss += self.stiefel_optim.step()
        loss += self.stiefel_block_optim.step()
        loss += self.spd_optim.step()
        loss += self.weight_vector_optim.step()
        return {'loss': loss}
